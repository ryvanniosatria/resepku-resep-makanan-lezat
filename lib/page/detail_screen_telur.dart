...
class DetailScreenTelur extends StatefulWidget {
  final ModelTelur telur;

  const DetailScreenTelur({Key? key, required this.telur}) : super(key: key);

  @override
  _DetailScreenTelurState createState() => _DetailScreenTelurState();
}

class _DetailScreenTelurState extends State<DetailScreenTelur> {
  ...
  Future<void> showCustomSnackBar(String message) async {
    ...
    Future<void> shareResep(ModelTelur resep) async {
  ...
  Future<void> simpanResep() async {
    ...
    String resepJson = jsonEncode(widget.telur.toJson());
    ...
    await showCustomSnackBar('Resep ${widget.telur.nama} telah disimpan');
    // Tambahkan pesan log atau penanganan lain yang diperlukan
    print(
        'Resep ${isResepDisimpan ? "disimpan" : "dihapus"}: ${widget.telur.nama}');
  }
  ...
  Future<void> hapusResep() async {
    ...
    await showCustomSnackBar('Resep ${widget.telur.nama} telah dihapus');
    // Tambahkan pesan log atau penanganan lain yang diperlukan
    print('Resep dihapus dari penyimpanan: ${widget.telur.nama}');
  }
          ...
          children: [
            ...
            Container(
              decoration: BoxDecoration(
                ...
                image: DecorationImage(
                  image: NetworkImage(widget.telur.gambar?.toString() ?? ''),
                  fit: BoxFit.cover,
                ),
              ),
              height: 200,
            ...
            Row(
              ...
              children: [
                // Waktu Masak
                buildInfoItem(Icons.access_time, widget.telur.waktuMasak ?? ''),
                // Porsi
                buildInfoItem(Icons.restaurant, widget.telur.porsi ?? ''),
                // Tingkat Kesulitan
                buildInfoItem(Icons.star, widget.telur.tingkatKesulitan ?? ''),
              ],
            ),
            SizedBox(height: 16.0),
