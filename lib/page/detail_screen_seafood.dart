...
class DetailScreenSeafood extends StatefulWidget {
  final ModelSeafood seafood;

  const DetailScreenSeafood({Key? key, required this.seafood}) : super(key: key);

  @override
  _DetailScreenSeafoodState createState() => _DetailScreenSeafoodState();
}

class _DetailScreenSeafoodState extends State<DetailScreenSeafood> {
  ...
  Future<void> showCustomSnackBar(String message) async {
    ...
    Future<void> shareResep(ModelSeafood resep) async {
  ...
  Future<void> simpanResep() async {
    ...
    String resepJson = jsonEncode(widget.seafood.toJson());
    ...
    await showCustomSnackBar('Resep ${widget.seafood.nama} telah disimpan');
    // Tambahkan pesan log atau penanganan lain yang diperlukan
    print(
        'Resep ${isResepDisimpan ? "disimpan" : "dihapus"}: ${widget.seafood.nama}');
  }
  ...
  Future<void> hapusResep() async {
    ...
    await showCustomSnackBar('Resep ${widget.seafood.nama} telah dihapus');
    // Tambahkan pesan log atau penanganan lain yang diperlukan
    print('Resep dihapus dari penyimpanan: ${widget.seafood.nama}');
  }
          ...
          children: [
            ...
            Container(
              decoration: BoxDecoration(
                ...
                image: DecorationImage(
                  image: NetworkImage(widget.seafood.gambar?.toString() ?? ''),
                  fit: BoxFit.cover,
                ),
              ),
              height: 200,
            ...
            Row(
              ...
              children: [
                // Waktu Masak
                buildInfoItem(Icons.access_time, widget.seafood.waktuMasak ?? ''),
                // Porsi
                buildInfoItem(Icons.restaurant, widget.seafood.porsi ?? ''),
                // Tingkat Kesulitan
                buildInfoItem(Icons.star, widget.seafood.tingkatKesulitan ?? ''),
              ],
            ),
            SizedBox(height: 16.0),
